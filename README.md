<p align="center">
  <img src="assets/system-map.svg" alt="Detailed system map for Obsidian" width="100%">
</p>
...
Tool integrations can create accidental side effects when authentication, scope, preview, execution, and verification are not separated. **Obsidian** turns that work into an explicit sequence with visible inputs, outputs, review points, and completion evidence.

## Why the repository has this name

The shared `hermes-skill-` prefix identifies this as a portable Hermes workflow package. `obsidian` names the capability directly—obsidian—so the repository remains searchable and understandable outside the original AI-OS workspace. The public title is **Obsidian**.
...
| What does it do? | Read, search, create, and edit notes in an Obsidian vault while preserving links and structure. |
...
![Obsidian system map](assets/system-map.svg)

## Operation sequence

![Obsidian actor and data sequence](assets/operation-sequence.svg)
...
![Illustrative output produced by Obsidian](assets/example-output.svg)
...
> In a disposable test workspace, read, search, create, and edit notes in an Obsidian vault while preserving links and structure. Return the result, the evidence used to verify it, and any limitations or actions that still require approval.
