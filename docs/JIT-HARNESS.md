...
This package expresses **Obsidian** as a typed, task-adaptive agent harness. Its concrete capability remains: Read, search, create, and edit notes in an Obsidian vault while preserving links and structure. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Obsidian |
| --- | --- | --- |
| Memory | `entity_relationship_memory` | Retain vault path plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `dependency_and_path_traversal` | Transform markdown notes into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_preview_authorize_execute_reread` | Advance the workflow through wiki links and tags while preserving stop conditions and user authority. |
